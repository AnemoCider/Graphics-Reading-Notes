...
还记得之前简单提过的Nanite occlusion culling吗？简单来说，就是绘制上一帧可见的cluster作为HZB。然而，由于LOD selection在帧与帧之间会有变化，加上streaming，上一帧可见的cluster可能已经不在memory里了。因此，我们转而测试这一帧的cluster，如果放在上一帧里，是否是可见的。具体来说：

- 这一帧的所有cluster使用上一帧的transform，和上一帧的HZB做test；
- 通过的绘制，不通过的存起来；（第一次绘制）
- 用绘制好的depth buffer构建这一帧的HZB；
- 把刚才没通过的拿来和新的HZB测试，通过就绘制；（第二次绘制）
- 完整的depth buffer生成HZB供下一帧使用。
...
artists有时会把instance count堆到polycount的数量级。这带来的一个问题就是，如何处理大量只覆盖了几个像素的mesh。在DAG上，这种mesh表现为仅仅一个cluster。如果采用merge的方式，重复instance导致的内存占用会是个问题。解决办法是使用visibility buffer imposter，也就是在12x12的view direction上分别构造imposter，储存depth和triangle ID，在运行时就可以直接注入到vis 
