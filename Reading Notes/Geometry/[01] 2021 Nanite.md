...
上述是基于原文提炼的，但是我怎么想都感觉这无法做到正确的culling。比如，假设每个group都有相同的

在看过GAMES104对应的lecture[^18]，以及翻阅了源码之后，我感觉talk里的说法并不是很准确。被赋予相同error和bounds的并不是group里的cluster，而是这些cluster(children)在merge & split后得到的parent cluster。同时，必须选择相同LOD的也是这些cluster。体现在源码里，group里的cluster会先被merge成一个大的cluster，然后simplify得到parent cluster，并估计这一过程引入的error（原文中说这是非常难的一步，因为完全没有纹理或材质信息）。这个error和所有children cluster自带的lodError取最大值，得到`ParentMaxLODError`并赋值给所有parent，以及这个group。
...
这样应当也是合法的。尽管1, 1, 3, 3属于同一个group，但他们之间的boundary在simplify那一步是被lock住的。
