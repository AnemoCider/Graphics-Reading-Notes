...
- 重复此流程，直到cluster能被merge到一个parent里。

LOD selection要保证一个group里的所有cluster都选取相同的LOD。然而，在运行时遍历巨大的DAG来做selection是不可接受的，所以我们需要找到一个局部的、因而可以并行的方法。具体的做法是，在build的时候让父节点的error不小于子节点的，这样一个threshold可以确定最多一个合理的cut。并且，只要threshold足够小，使得error是subpixel级别的，那么父子节点之间的切换过程就会被TAA掩盖掉。

有了局部的判断条件，我们就可以根据DAG建立一个BVH，使得每个node的children都是它在DAG里的descendants。

<!-- 由于LOD0的cluster group是不可能只被部分选中的，最底层的cluster就以group为单位作为leaf node。 -->

具体的LOD culling做法是用固定数量的线程+任务队列：每个线程拿出一个node，做culling，通过了就把children加入。

上述是基于原文提炼的，但是我最开始感觉这个过程并不能实现之前的思路。首先抛出几个我遇到的问题：

- 文中说，为了保证一个group会选择同一LOD，需要给group里的cluster赋予相同的error和bounds。然而，我完全可以设置如下的error，得到一个不合法的selection:

**GAMES104图，或者手绘**

- DAG cut和合法的selection之间有任何对应关系吗？我可以找出一个没有对应selection的cut，也可以反过来：

**GAMES104图，看似合法的cut**

**uiuc图，合法的selection，不合法的cut**

在看过GAMES104对应的lecture[^18]，以及翻阅了源码之后，我感觉talk里的说法并不是很准确。被赋予相同error和bounds的并不是group里的cluster，而是这些cluster(children)在merge & split后得到的parent cluster。体现在源码里，group里的cluster会先被merge成一个大的cluster，然后simplify得到parent cluster，并估计这一过程引入的error（原文中说这是非常难的一步，因为完全没有纹理或材质信息）。这个error和所有children cluster自带的lodError取最大值，得到`ParentMaxLODError`并赋值给所有parent，以及这个group。

？？但这里仍然有问题，这样的话parent那个group里的error是不一样的，那怎么保证parent group选取相同的LOD??

`Encode`负责构建BVH，树上的节点为`FHierarchyNode`(HNode)。叶子节点的HNode包含了一些来自同一个group的cluster，它的LODBounds和MaxParentLODError也是根据group来的。非叶子结点的HNode的这两个值是其所有子节点的Union/最大值。LOD selection过程中，遍历到某一个HNode进行LODCulling的时候（`ProcessNodeBatch`），如果ParentError已经比threshold要小了，就可以剔除掉这个HNode。否则，会把其所有子节点加入队列。对于叶子HNode，会把cluster入队。这样一来，所有最终入队的cluster都满足了$$ParentError > threshold$$的条件。随后，在`ClusterCull`的时候，会判断`SmallEnoughToDraw`，也就是$$clusterError <= threshold$$。通过则不被LOD cull。实际上，还有一个可用的剔除条件是，在ParentError过大剔除HNode。但是，由于ParentError是一层层向上取max的，如果要额外引入这个条件，就很难做到目前快速且保守的、对于LOD低（过于细节）的group的剔除。测试发现LOD过低的group远多于LOD过高的，所以就不引入这个条件了。

我们分析一下这为什么能完成LOD Cull的目标：

- Group级别的Cull是基于HNode进行的。同一个group对应的那些HNode用的都是group的ParentError，要么都被cull，要么都留下来。
- 最关键的一点是，HNode culling和cluster culling用的都是ParentError，这保证了能通过LOD culling的cluster，它的children都会在HNode culling的时候被剔除。
...
在`NaniteHierarchyTraversal.ush`里可以找到`PersistentNodeAndClusterCull`这个函数。
